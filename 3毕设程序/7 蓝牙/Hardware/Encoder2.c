...
{
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4,ENABLE); //启用或禁用APB1 (Low Speed APB)外设时钟
	...
	TIM_TimeBaseInit(TIM4,&TIM_TimeBaseInitStructure); //初始化TIMx时间基准单元外设
	...
	TIM_ICInit(TIM4,&TIM_ICInitStructrue);//初始化TIM外设
		TIM_ICInitStructrue.TIM_Channel=TIM_Channel_2;//指定TIM通道
		TIM_ICInitStructrue.TIM_ICFilter=0xF; //指定输入捕获过滤器
		TIM_ICInit(TIM4,&TIM_ICInitStructrue);//初始化TIM外设
		
	TIM_EncoderInterfaceConfig(TIM4,TIM_EncoderMode_TI12,TIM_ICPolarity_Rising,TIM_ICPolarity_Rising);//配置TIMx编码器接口
		                                                                                              //Rising~就是通道不反向
	TIM_Cmd(TIM4,ENABLE);//启用或禁用指定的TIM外设																								  
}

int16_t Encoder2_Get(void)//reference参考
{
    int16_t Temp;
